...
export default async function BulkSessionRegistration(tickets, token, eventId, sessionId) {
  ...
  for (let i = 0; i < tickets.length; i++) {
    ...
    try {
      ...
      if (res.status === 200) {
        console.log(res.status, chalk.bgCyan('Ticket'), chalk.rgb(255,165,0).underline(`${ticket['Ticket Number']}`), `registered to session #${sessionId}`)
      }
      if (res.status === 401) {
        return 'BAD_TOKEN'
      }
